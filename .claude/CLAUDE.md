...
- **CI** (`.github/workflows/ci.yml`): Builds, lints, and tests on PRs and main pushes. Uploads binary as artifact. Also exports `samples/sample_session.jsonl` and deploys to GitHub Pages. CI also captures screenshots of demo exports for both themes and generates a showcase landing page (`npm run capture-showcase`, `npm run generate-showcase`). Main pushes publish `index.html` + `{short-sha}.html`; PR builds publish `pr-{number}.html`. All files persist via `keep_files: true` so previous versions remain accessible.
